...
- **Admin access**: superusers use a **regular password** (the standard `createsuperuser` flow — `create_superuser(..., password=...)` sets a usable password; member accounts stay passwordless). The Django admin (`/admin/`) uses the standard login form and is restricted to **logged-in superusers only** via `apps/accounts/admin_site.py::SuperuserOnlyAdminSite` (wired through `apps/accounts/admin_config.py::AccountsAdminConfig` as `default_site` in `config/settings.py`; `has_permission` requires `is_active and is_superuser`). Non-superuser staff are redirected to the admin login.
