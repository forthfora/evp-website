...
- **django-ratelimit** — all API endpoints are rate-limited (per-IP or per-user-or-IP); see decorators on each route. Counts live in the default Django cache: **Redis** (shared across Gunicorn workers) when `CACHE_URL` is set, falling back to per-process `LocMemCache` when unset. **⚠️ Neither compose file sets `CACHE_URL`** (it is commented out in `backend/.env.example`) — local dev runs on `LocMemCache` unless it is enabled in `backend/.env`, and prod is only Redis-backed if the server's root `.env` sets it (see Known Issues). Both compose files do run a `redis:7-alpine` service, ready to back it
...
- CI uses **Node 22** for frontend, **Python 3.13** for backend — while the frontend `Dockerfile` builds with `node:24-alpine` (mismatch, see Known Issues)
...
- **Env vars**: in local dev the backend reads from `backend/.env` (python-decouple; the dev compose mounts `./backend` to `/app` so the file is found there). In production, `docker-compose.prod.yml` uses `env_file: .env` at the **repo root** on the server. Neither compose file sets `CACHE_URL` — to get Redis-backed (cross-worker) rate limits, set `CACHE_URL=redis://redis:6379/0` (not a secret) in `backend/.env` (dev) or the server's root `.env` (prod); otherwise the per-process `LocMemCache` fallback applies. Never commit `.env` (note: there is no root `.gitignore` — see Known Issues).
...
Findings from the 2026-09 project-wide reviews (latest pass: 2026-09-05). Previously listed items (Python 3.12 Dockerfile, missing `MEDIA_URL`/`MEDIA_ROOT`, unused `PyJWT`/`django-redis`, SMTP dead code, `docs/adr/` references, unused `/admin/` Nginx proxy, contact-form HTML injection, per-process rate limits, OTP codes stored plaintext, unsanitised Markdown in admin update emails, missing HSTS/security headers) have been resolved and removed. The email-schema fix was **partial**: `RequestOTPIn`/`VerifyOTPIn`/`EmailChangeIn` use pydantic `EmailStr`, but `ContactIn.email` and the startupdb `FounderIn`/`StartupIn` `email`/`website`/`linkedin` fields are still plain `str` with no length limits, and `FounderIn.occupation` does not enforce the model's choices — oversized values can surface as MySQL `DataError` → 500.

### Security

- **`CACHE_URL` is set in neither compose file** (contrary to earlier docs): the dev compose has no `environment:`/`env_file:` for the backend at all, and prod only has `env_file: .env`; it is commented out in `backend/.env.example`. Consequence: dev rate limits are per-process `LocMemCache`, and prod limits are only Redis-backed (shared across Gunicorn workers) if the server's root `.env` sets `CACHE_URL` — otherwise the OTP/auth brute-force protections are per-worker.
- **`backend/.dockerignore` does not exclude `.env`** — `COPY . .` in `backend/Dockerfile` bakes the local `backend/.env` (real `SECRET_KEY` + Resend API key) into any locally built image. CI-built images are unaffected (the file is not committed), but the Resend key should be rotated if local images were ever pushed/shared. Add `.env` to `.dockerignore`.
...
- **OTP re-issue does not invalidate prior unconsumed codes** — a new request shadows the previous code (minor login-DoS vector), and lockout is per-record: 5 requests + 10 verifies per 10 min per IP allows ~50 guesses/10 min/IP against a 10⁶ code space, parallelisable across IPs. `sendall`'s subject is also unbounded (the body is sanitised and the Resend JSON API rules out header injection, but there is no length limit).

### Correctness / robustness

- **`verify_otp` race**: `User.objects.get_or_create(email=...)` can raise `IntegrityError` under concurrent verifies of the same new email → 500. The `get_or_create` also bypasses `UserManager.create_user` (brief `password=""` window before `set_unusable_password()`).
- **PATCH endpoints accept explicit `null` → 500**: `MePatchIn`/`FounderPatchIn`/`StartupPatchIn` use `str | None = None` + `exclude_unset=True`, so an explicitly-sent `{"first_name": null}` counts as "set" and assigns `None` to non-nullable `CharField`s → `IntegrityError`.
- **No pagination** on `GET /api/accounts/members`, `GET /api/startupdb/`, `GET /api/startupdb/founders` (full-table serialisation); `sendall` does one greeting-lookup query per recipient (N+1); the startupdb list endpoints also N+1 on `created_by` (no `select_related`).
- **`sendall` sends synchronously in-request** — with more than a handful of recipients, Resend round-trips can exceed Gunicorn's 30 s worker timeout → 502 mid-send (partial delivery, no resume).
- **Email case-sensitivity inconsistency**: `request_otp`/`verify_otp` filter `email` (exact) while `_build_greeting` uses `email__iexact`, and nothing lowercases emails — `User@x.com` vs `user@x.com` are distinct accounts on SQLite but collide on MySQL's default collation (divergent dev/prod behaviour).
...
- **`robots.txt` has a `Diallow: /evp-dev/` typo** — the directive is invalid, so the admin path is not actually disallowed (`/api/` and `/member` are correctly disallowed; the stale `/admin/`+`/dashboard/` entries are gone). `sitemap.xml` now includes `/join`, `/privacy`, `/terms` (previously missing — fixed).
...
- **Root `package.json` is vestigial** (only `class-variance-authority`; no scripts/workspaces) — and the resulting root `node_modules/` (~20 files) and `package-lock.json` are **committed to git**. `cva` is already a dependency in `frontend/package.json`; the root manifest and its artifacts should be removed.
...
- **Node version mismatch**: the frontend `Dockerfile` builds with `node:24-alpine` while CI (`deploy.yml`) uses **Node 22**.
- **CI never runs the frontend Vitest suite** — the frontend job is `npm ci` + lint + build only. The frontend `Dockerfile` also uses `npm install` (not `npm ci`), and neither Dockerfile defines a `HEALTHCHECK` (containers run as root).
...
- **Minor cleanliness**: user-facing typo "occured" in the 500 message (`apps/accounts/api.py` + `apps/core/api.py`); stale Django 4.2 docstring in `config/urls.py`; unnecessary `sys.path.insert` hack in `config/settings.py`; `MeOut`/`MemberOut` and `_me_out`/`_member_out` are field-identical duplicates.
